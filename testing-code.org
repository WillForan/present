...
#+begin_src R :session
add3 <- function(x) x +3
stopifnot(add3(5) == 8, "add3 doesn't work as expected")
#+end_src

#+RESULTS:
: Error: "add3 doesn't work as expected" is not TRUE

*** checking input
#+begin_src R :session
library(LNCDR)
ex_df <- data.frame(a=1:26,b=letters)
# check fisrt column is integer, second is strings
with(ex_df, qassertm(a="i+", b="s"))
#+end_src

#+RESULTS:

#+begin_src R :session
with(ex_df, qassertm(a="i+", b="i"))
#+end_src

#+RESULTS:
: Error in eval(substitute(checkmate::qassert(x, spec), list(x = as.name(varname),  :
:  Assertion on 'b' failed. Must be of class 'integer', not 'character'.
** Workshop?
...
** How to organize these tests?

  * test functions in notebooks?
  * separate *test.R files, run by testthat
...
: 1..2
: ok 1 lunaid_test
: ok 2 lunaid_date_test

** in lncdtools
https://github.com/lncd/lncdtools/blob/master/ld8

** All in one
#+begin_src bash :tangle ./testing-code/allinone.bash :eval never
getlunaid() { grep -o '\d{5}'; }
eval "$(iffmain runall)"
...
#+begin_src bash :dir testing-code
#+end_src
